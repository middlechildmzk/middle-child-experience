...
const releases = [
  ...
  { title: 'Mercy', detail: 'Middle Child · Catalog', href: 'https://open.spotify.com/artist/2hp8yAzOnYRUFMCdot9tzN', image: 'https://i.ytimg.com/vi/9bCVDn2P29Q/maxresdefault.jpg' },
          ...
          <div className="grid">
            {releases.map((r, i) => (
              <a className="card release-card" href={r.href} key={r.title} target={r.href.startsWith('http') ? '_blank' : undefined} rel="noreferrer">
                ...
                <div>
                  <p className="eyebrow">{i === 0 ? 'Featured release' : 'Catalog'}</p>
                  <h2>{r.title}</h2>
                  <p>{r.detail}</p>
                </div>
              </a>
